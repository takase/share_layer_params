...
class TransformerModel(FairseqEncoderDecoderModel):
    ...
    def add_args(parser):
        ...
        #arguments for training upper layer only
        parser.add_argument('--share-params-cross-layer', default=False, action='store_true',
                            help='share parameters cross layers such as ALBERT')
        parser.add_argument('--share-layer-num', default=-1, type=int, 
                            help='the number of layers for parameter sharing if share parameters cross layers, -1 means share all layers')
        parser.add_argument('--share-type', default='sequence', choices=['sequence', 'cycle', 'cycle_reverse'],
                            help='the type of sharing layers')
        # fmt: on
        ...
        if self.share_params_cross_layer:
            if self.share_type == 'sequence':
                ...
                for i in range(args.encoder_layers):
                    ...
                    shared_num += 1
                    if self.share_layer_num != -1 and shared_num == self.share_layer_num:
                        shared_num = 0
            else:
                ...
                if self.share_type == 'cycle':
                    for i in range(args.encoder_layers - unique_layer_num):
                        encoder_layer = TransformerEncoderLayer(args, LayerNum=i + unique_layer_num)
                        encoder_layer = CopyParams(self.layers[i % unique_layer_num], encoder_layer)
                        self.layers.extend([encoder_layer])
                    ...
                    for i in range(args.encoder_layers - len(self.layers)):
                        encoder_layer = TransformerEncoderLayer(args, LayerNum=len(self.layers))
                        encoder_layer = CopyParams(base_layers[-((i % unique_layer_num) + 1)], encoder_layer)
                        self.layers.extend([encoder_layer])
        else:
            self.layers.extend([
        ...
        if self.share_params_cross_layer:
            if self.share_type == 'sequence':
                ...
                for i in range(args.decoder_layers):
                    if shared_num == 0:
                        base_decoder_layer = TransformerDecoderLayer(args, no_encoder_attn, LayerNum=i)
                        self.layers.extend([base_decoder_layer])
                    else:
                        decoder_layer = TransformerDecoderLayer(args, no_encoder_attn, LayerNum=i)
                        decoder_layer = CopyParams(base_decoder_layer, decoder_layer, decoder=True)
                ...
                for i in range(unique_layer_num):
                    self.layers.extend([TransformerDecoderLayer(args, no_encoder_attn, LayerNum=i)])
                if self.share_type == 'cycle':
                    for i in range(args.decoder_layers - unique_layer_num):
                        decoder_layer = TransformerDecoderLayer(args, no_encoder_attn, LayerNum=i + unique_layer_num)
                        decoder_layer = CopyParams(self.layers[i % unique_layer_num], decoder_layer, decoder=True)
                        self.layers.extend([decoder_layer])
                elif self.share_type == 'cycle_reverse':
                    ...
                    for i in range(args.decoder_layers - len(self.layers)):
                        decoder_layer = TransformerDecoderLayer(args, no_encoder_attn, LayerNum=len(self.layers))
                        decoder_layer = CopyParams(base_layers[-((i % unique_layer_num) + 1)], decoder_layer, decoder=True)
                        self.layers.extend([decoder_layer])
        else:
            self.layers.extend([
...
def CopyParams(base_layer, target_layer, decoder=False):
    ...
    if decoder:
        if target_layer.encoder_attn.qkv_same_dim:
            target_layer.encoder_attn.in_proj_weight = base_layer.encoder_attn.in_proj_weight
        else:
            target_layer.encoder_attn.k_proj_weight = base_layer.encoder_attn.k_proj_weight
            target_layer.encoder_attn.v_proj_weight = base_layer.encoder_attn.v_proj_weight
            target_layer.encoder_attn.q_proj_weight = base_layer.encoder_attn.q_proj_weight
